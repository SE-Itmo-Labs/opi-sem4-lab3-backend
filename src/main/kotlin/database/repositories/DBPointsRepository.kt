...
open class DBPointsRepository : JpaRepository<Point2DRow, Long>() {
    ...
    open fun findAllByUser(userId: Long): List<Point2DRow> =
        entityManager
            .createQuery("SELECT p FROM Point2DRow p WHERE p.user.id = :userId", Point2DRow::class.java)
    ...
    open fun getAllPoints(): List<Point2DRow> =
        entityManager
            .createQuery("SELECT p FROM Point2DRow p ORDER BY p.timestamp DESC", Point2DRow::class.java)
            .resultList

    @Transactional
    open fun deleteAllByUser(userId: Long) =
        entityManager
            .createQuery("DELETE FROM Point2DRow p WHERE p.user.id = :userId")
            .setParameter("userId", userId)
            .executeUpdate()
